...
class Int8CheckpointStore:
    ...
    def __init__(
        ...
        *,
        ...
        self.qdata = torch.empty(
            num_layers, num_slots, num_heads, head_v_dim, head_k_dim,
            dtype=torch.int8, device=device,
        )
        self.scale = torch.empty(
            num_layers, num_slots, num_heads, 1, head_k_dim,
            dtype=scale_dtype, device=device,
        )
    ...
    def quantize(cls, state: torch.Tensor):
        ...
        amax = state.abs().amax(dim=-2, keepdim=True).clamp(min=1e-8)
        scale = amax / cls.QMAX
        q = torch.round(state / scale).clamp(-cls.QMAX, cls.QMAX).to(torch.int8)
        return q, scale
    ...
    def mem_usage_bytes(self) -> int:
        return self.qdata.numel() + self.scale.numel() * self.scale.element_size()
