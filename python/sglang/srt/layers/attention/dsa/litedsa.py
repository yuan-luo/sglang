...
Enabled by ``SGLANG_LITEDSA=1`` (G auto = 128 // num_local_heads,
...
_MAX_SEQ_SPACE = 1 << 20
_DYNAMIC_SEQ_SPACE = os.environ.get("SGLANG_LITEDSA_DYNAMIC_SPAN", "1") == "1"
_UNION_SO = os.environ.get("SGLANG_LITEDSA_UNION_SO")
_UNION_MOD = None
# SGLANG_LITEDSA_SO: load BOTH litedsa ops (union + masked attention) from a
# TVM-FFI module instead of torch.ops._C — lets a freshly built kernel ship
# without rebuilding the whole vLLM extension.
_LITEDSA_SO = os.environ.get("SGLANG_LITEDSA_SO")
_LITEDSA_MOD = None
...
def _union_seq_space(max_seq_len: int) -> int:
    ...
    if max_seq_len <= 0 or max_seq_len > _MAX_SEQ_SPACE:
        raise ValueError(
            f"LiteDSA max_seq_len must be in (0, {_MAX_SEQ_SPACE}], "
            f"got {max_seq_len}"
        )
...
_UNION_STATS = os.environ.get("SGLANG_LITEDSA_UNION_STATS", "0") == "1"
_REUSE_OUTPUT_BUFS = os.environ.get("SGLANG_LITEDSA_REUSE_OUTPUT_BUFS", "0") == "1"
...
def _reuse_output_buffers_allowed() -> bool:
    ...
    if torch.cuda.is_current_stream_capturing():
        raise RuntimeError(
            "SGLANG_LITEDSA_REUSE_OUTPUT_BUFS is not capture-safe; "
