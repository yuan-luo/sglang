...
__global__ void litedsa_union_qm_kernel(
...

  {
    ...
    for (int i = tid; i < s_words / 4; i += kLiteDSAUnionThreads)
      b4[i] = z;
    if (tid < kLiteDSAUnionThreads / 32)
      summary[tid] = 0u;
  }
  __syncthreads();
...
#pragma unroll
    ...
    if (lane < kLiteDSAUnionThreads / 32)
      warp_sums[lane + 1] = wv;
  }
  __syncthreads();
