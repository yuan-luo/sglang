...
static inline CUtensorMap
make_tensor_map(const std::vector<uint64_t> &size,
...
make_stride_helper(const std::vector<T> &strides_in_elems, size_t elem_size) {
...
void tma_gather4(const void *desc_ptr, transac_bar_t &mbar_ptr, void *smem_ptr,
  ...
  asm volatile("cp.async.bulk.tensor.2d.shared::cta.global.tile::gather4."
               "mbarrier::complete_tx::bytes.cta_group::1.L2::cache_hint [%0], "
               "[%1, {%2, %3, %4, %5, %6}], [%7], %8;\n"
               :
               : "r"(smem_addr), "l"(desc_ptr), "r"(col_idx), "r"(row_idxs.x),
                 "r"(row_idxs.y), "r"(row_idxs.z), "r"(row_idxs.w),
...
void tma_gather4_prefetch(const void *desc_ptr, int col_idx, int4 row_idxs,
                          int64_t cache_hint) {
  asm volatile("cp.async.bulk.prefetch.tensor.2d.L2.global.tile::gather4.L2::"
               "cache_hint [%0, {%1, %2, %3, %4, %5}], %6;\n"
               :
...
CUTE_DEVICE void tma_gather4_cta_group_2(const void *desc_ptr,
  ...
  asm volatile("cp.async.bulk.tensor.2d.shared::cta.global.tile::gather4."
               "mbarrier::complete_tx::bytes.cta_group::2.L2::cache_hint [%0], "
               "[%1, {%2, %3, %4, %5, %6}], [%7], %8;\n"
               :
               : "r"(smem_addr), "l"(desc_ptr), "r"(col_idx), "r"(row_idxs.x),
                 "r"(row_idxs.y), "r"(row_idxs.z), "r"(row_idxs.w),
...
struct Copy_Traits<SM100_TMA_2SM_LOAD_NOSPLIT, NumBitsPerTMA, AuxParams_> {
  ...
  copy_unpack(Copy_Traits const &traits, Tensor<TS, SLayout> const &src,
...
struct SparseAttnDecodeParams {
  ...
  cutlass::bfloat16_t
      *__restrict__ extra_kv; // [extra_num_blocks, extra_page_block_size, d_qk]
  ...
  DecodingSchedMeta
      *__restrict__ tile_scheduler_metadata_ptr; // [num_sm_parts, ], contiguous
...
struct SM100_MMA_F8F6F4_2x1SM_SS_NOELECT {
  ...
  fma(uint64_t const &desc_a, uint64_t const &desc_b, uint32_t const &tmem_c,
    ...
    asm volatile("{\n\t"
                 ".reg .pred p;\n\t"
                 "setp.ne.b32 p, %4, 0;\n\t"
                 "tcgen05.mma.cta_group::2.kind::f8f6f4 [%0], %1, %2, %3, {%5, "
                 "%6, %7, %8, %9, %10, %11, %12}, p; \n\t"
                 "}\n"
                 :
                 : "r"(tmem_c), "l"(desc_a), "l"(desc_b),
                   "r"(uint32_t(idescE >> 32)), "r"(scaleC), "r"(mask[0]),
                   "r"(mask[1]), "r"(mask[2]), "r"(mask[3]), "r"(mask[4]),
                   "r"(mask[5]), "r"(mask[6]), "r"(mask[7]));
#else
    CUTE_INVALID_CONTROL_PATH(
...
template <int D_QK> struct KernelTemplate {
  ...
  sparse_attn_fwd_kernel_devfunc(const SparseAttnFwdParams &params,
...
__device__ void KernelTemplate<D_QK>::sparse_attn_fwd_kernel_devfunc(
  ...
  if (warpgroup_idx == 0) {
    ...
    uint128_t *sS_base0 = (uint128_t *)plan.s[0].data() +
                          idx_in_warpgroup % 64 +
                          64 * ((idx_in_warpgroup / 64) * 4);
    uint128_t *sS_base1 = (uint128_t *)plan.s[1].data() +
                          idx_in_warpgroup % 64 +
                          64 * ((idx_in_warpgroup / 64) * 4);
    ...
    for (int k = 0; k < num_k_blocks; ++k) {
      if (qm_row != nullptr && k + 1 < num_k_blocks)
        qm_nx =
            __ldg((const uint2 *)(qm_row + (size_t)(k + 1) * (B_TOPK / 32)));
