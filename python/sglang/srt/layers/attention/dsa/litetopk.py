...
Env knobs:
  ...
  SGLANG_LITETOPK_DENSE_SELECT_MIN_S
                              first dense length to replace (default 40960)
  SGLANG_LITETOPK_DENSE_SELECT_MAX_S
                              last dense length to replace (default 262144)
  SGLANG_LITETOPK_DENSE_SELECT_BINS
                              exact coarse histogram bins (fixed at 4096)
  SGLANG_LITETOPK_DENSE_SELECT_MIN_LOGITS_MB
  ...
  SGLANG_LITETOPK_CHECK=1      also run the official path and log top-k recall
  SGLANG_LITETOPK_DEDUP_CARRY_WAIT
                              elide an already-satisfied carry event wait (default 1)
  SGLANG_LITETOPK_MERGE_CAP    per-row candidate capacity (default 196608)
  SGLANG_LITETOPK_HEADROOM     bucket-scale headroom (default 0)
  SGLANG_LITETOPK_OVF_LOG=1    log new candidate-count maxima
  SGLANG_LITETOPK_PROBE_EVERY  overflow telemetry cadence (default 8); probed
                              chunks also fail closed on selector status
  SGLANG_LITETOPK_OVF_WATERMARK
...
# Keep the original 8K qualification shapes while also accepting the
# checked-in SGLang cookbook chunks.  The tails are the corresponding shapes
# for the 1,048,512-token qualification corpus (full context minus 64 tokens).
FUSED_QUERY_LEN = 8192
FUSED_TAIL_QUERY_LEN = 8128
FP8_FUSED_QUERY_LENS = frozenset((FUSED_QUERY_LEN, FUSED_TAIL_QUERY_LEN, 16384, 16320))
...
def supports_fused_query_len(query_length: int, *, use_fp4: bool) -> bool:
    ...
    supported = FP4_FUSED_QUERY_LENS if use_fp4 else FP8_FUSED_QUERY_LENS
    return int(query_length) in supported
...
def enforce_required_path(
    *,
    ...
    if not supports_fused_query_len(query_length, use_fp4=use_fp4):
        supported = sorted(FP4_FUSED_QUERY_LENS if use_fp4 else FP8_FUSED_QUERY_LENS)
        reasons.append(
...
HOT_PREFIX = 12288
DENSE_SELECT = os.environ.get("SGLANG_LITETOPK_DENSE_SELECT", "1") == "1"
DENSE_SELECT_MIN_S = int(os.environ.get("SGLANG_LITETOPK_DENSE_SELECT_MIN_S", "40960"))
DENSE_SELECT_MAX_S = int(os.environ.get("SGLANG_LITETOPK_DENSE_SELECT_MAX_S", "262144"))
DENSE_SELECT_BINS = int(os.environ.get("SGLANG_LITETOPK_DENSE_SELECT_BINS", "4096"))
DENSE_SELECT_MIN_LOGITS_MB = int(
    os.environ.get("SGLANG_LITETOPK_DENSE_SELECT_MIN_LOGITS_MB", "0")
)
if DENSE_SELECT_BINS != 4096:
    raise ValueError("SGLANG_LITETOPK_DENSE_SELECT_BINS must be 4096")
...
if DENSE_SELECT_MIN_LOGITS_MB < 0:
    raise ValueError("SGLANG_LITETOPK_DENSE_SELECT_MIN_LOGITS_MB must be >= 0")
NB = int(os.environ.get("SGLANG_LITETOPK_NB", "256"))
CHECK = os.environ.get("SGLANG_LITETOPK_CHECK", "0") == "1"
...
DEDUP_CARRY_WAIT = os.environ.get("SGLANG_LITETOPK_DEDUP_CARRY_WAIT", "1") == "1"
_HOT_CARRY = {}
...
# SGLANG_LITETOPK_PATH_TIMING=1: CUDA-event sub-segment timing of the fused
# exact-once chain (seed | scan | select | map+vote+probe), keyed by 64K band
# of S. Lazy readback of completed pairs only — sync-free.
_SEG_ON = os.environ.get("SGLANG_LITETOPK_PATH_TIMING", "0") == "1"
...
def _seg_commit(seq_len, evs):
    ...
    while rec["pend"] and rec["pend"][0][-1].query():
        ...
        if rec["n"] % 128 == 0:
            ...
            print(
                f"[litetopk] {key}: n={rec['n']} " f"seed|scan|select|map_ms=[{segs}]",
                flush=True,
            )
...
def _deferred_overflow_poll():
    ...
    if _PENDING_OVF is not None:
        ev, pinned, capv, kk, watermark = _PENDING_OVF
...
_CARRY_TIMING = os.environ.get("SGLANG_LITETOPK_CARRY_TIMING", "0") == "1"
...
_CARRY_EVERY = max(1, int(os.environ.get("SGLANG_LITETOPK_CARRY_EVERY", "1")))
_CARRY_SKIP_COUNTS: dict = {}
_CARRY_IO_ENV = os.environ.get("SGLANG_LITETOPK_CARRY_IO", "1") == "1"
    ...
    *,
    ...
    try:
        ...
        if (
            ...
            or not (production_min_s(use_fp4) <= S <= PRODUCTION_MAX_S)
        ...
        if (
            not carry_valid
            and os.environ.get("SGLANG_LITETOPK_COLDSTART_IDENTITY", "0") != "1"
        ):
            if os.environ.get("SGLANG_LITETOPK_CARRY_DEBUG", "0") == "1":
                if carry is None:
                    why = "missing"
                else:
    ...
    try:
        ...
        def _check_static_stage(stage):
            if not diagnostic_stages:
                return
            diagnostic_status = int(status.max().item())
            if diagnostic_status == 0:
                return
            diagnostic_count = int(candidate_count.max().item())
            ...
            raise RuntimeError(
                ...
                f"{stage} status={diagnostic_status} "
                f"({status_help}); "
                f"S={S}, common_end={common_end}, cap={cap_eff}, "
                f"candidate_max={diagnostic_count}, layer={hot_key}"
            )
        ...
        if _PENDING_OVF is None and (
            ...
            event.record()
            _PENDING_OVF = (event, pinned, cap_eff, topk, OVF_WATERMARK)
