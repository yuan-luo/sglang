...
__global__ __launch_bounds__(Threads) void coarse_tiering_topk_kernel(
  ...
  for (int i = tid; i < kCoarseBins; i += Threads)
    hist[i] = 0u;
  __syncthreads();
...
#pragma unroll
  for (int pass = 0; pass < 3; ++pass) {
    for (int i = tid; i < kBins; i += Threads)
      hist[i] = 0u;
    __syncthreads();
...
select_histogram_bin(const int *histogram, int target, int *warp_prefix,
                     int *selected_bin, int *selected_count_lt) {
...
__global__ __launch_bounds__(kThreads) void overflow_exact_topk_kernel(
  ...
  if ((input_status & kBoundaryOverflow) == 0u)
    return;
...
candidate_options(const torch::TensorOptions &options) {
...
void gather_hot_sample_litetopk_(torch::Tensor k, torch::Tensor k_scale,
  ...
  TORCH_CHECK(idx.scalar_type() == torch::kLong ||
  ...
  if (idx.scalar_type() == torch::kLong) {
    gather_hot_sample_litetopk_kernel<<<blocks, kThreads, 0, stream>>>(
        ...
        k_scale.data_ptr<float>(), idx.data_ptr<int64_t>(),
        reinterpret_cast<uint4 *>(out_k.data_ptr()),
        out_scale.data_ptr<float>(), k.size(0), hot_n);
  } else {
    gather_hot_sample_litetopk_kernel<<<blocks, kThreads, 0, stream>>>(
        reinterpret_cast<const uint4 *>(k.data_ptr()),
        k_scale.data_ptr<float>(), idx.data_ptr<int32_t>(),
        reinterpret_cast<uint4 *>(out_k.data_ptr()),
        out_scale.data_ptr<float>(), k.size(0), hot_n);
  }
  C10_CUDA_KERNEL_LAUNCH_CHECK();
...
__global__ void seed_bcount_kernel(const float *__restrict__ seed_val,
  ...
  for (int i = tid; i < seed_k; i += blockDim.x) {
    ...
    if (braw < NB)
      atomicAdd(&bcount[(size_t)row * NB + b], 1);
  }
}

__global__ void
refresh_threshold_from_bcount_kernel(int32_t *__restrict__ th_bucket,
                                     const int32_t *__restrict__ bcount, int R,
                                     int NB, int K) {
  ...
  if (found && new_th < old_th)
    th_bucket[row] = new_th;
}
...
__global__ void carry_votes_emit_reset_litetopk_kernel(
  ...
  if constexpr (FusePlanner) {
    ...
    for (int rank = block * kCarryThreads + tid; rank < pair_count;
      ...
      const int a = static_cast<int>(reinterpret_cast<volatile int64_t *>(
          out_idx)[selected_inside + rank]);
...
compact_topk_paged_is_boundary(const uint32_t score_code, const int th,
...
static int compute_smem_bytes_t(bool include_hist = true) {
  ...
  const int smem_hist = include_hist ? kImplBlockQ * 256 * (int)sizeof(int32_t)
