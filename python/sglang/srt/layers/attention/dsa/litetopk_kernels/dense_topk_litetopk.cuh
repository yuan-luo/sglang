...
dense_topk_visit_row(const float *row, int begin, int end, int stride1, Fn fn) {
  if (stride1 == 1) {
...
__device__ __forceinline__ void
dense_topk_write_streaming_result(int32_t *row_out, const int32_t *selected,
                                  int topk, int free_k, int prefix_len,
                                  int suffix_len, int row_len) {
...
inline void
dense_hist_meta_litetopk_(torch::Tensor logits, torch::Tensor row_starts,
                          torch::Tensor row_ends, torch::Tensor threshold,
                          torch::Tensor count_lt, torch::Tensor count_eq,
                          int64_t rows, int64_t stride0, int64_t stride1,
...
inline void
dense_prehist_select_litetopk_(torch::Tensor logits, torch::Tensor row_starts,
                               torch::Tensor row_ends, torch::Tensor threshold,
                               torch::Tensor count_lt, torch::Tensor count_eq,
                               torch::Tensor out, int64_t rows, int64_t stride0,
                               int64_t stride1, int64_t topk, int64_t bins) {
  dense_topk_check_inputs(logits, row_starts, row_ends, rows, stride0, stride1,
                          topk);
  dense_topk_check_metadata(logits, threshold, count_lt, count_eq, rows);
  TORCH_CHECK(bins == 512 || bins == 1024 || bins == 2048 || bins == 4096,
