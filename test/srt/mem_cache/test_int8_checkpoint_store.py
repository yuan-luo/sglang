...
class TestInt8CheckpointCodec(unittest.TestCase):
    ...
    def test_store_load_roundtrip(self):
        store = Int8CheckpointStore(
            num_layers=L, num_slots=16, num_heads=H, head_v_dim=V, head_k_dim=K,
            device="cpu",
        )
        s = _rand_state(4)
        ...
        q, scale = Int8CheckpointStore.quantize(s)
        ref = Int8CheckpointStore.dequantize(q, scale.to(store.scale.dtype), torch.float32)
    ...
    def test_memory_is_half_of_bf16(self):
        store = Int8CheckpointStore(
            num_layers=L, num_slots=100, num_heads=H, head_v_dim=V, head_k_dim=K,
            device="cpu",
        )
    ...
    def test_decode_error_within_bound(self):
        ...
        def decode(state, inp):
            ...
            o, _ = fused_recurrent_kda(
                q=inp[0], k=inp[1], v=inp[2], g=inp[3], beta=inp[4], scale=K ** -0.5,
        ...
        pre = synth(512, 0)
        fused_recurrent_kda(q=pre[0], k=pre[1], v=pre[2], g=pre[3], beta=pre[4],
                            scale=K ** -0.5, initial_state=S, inplace_final_state=True,
                            use_qk_l2norm_in_kernel=True, cu_seqlens=None)
        dec = synth(128, 1)
        o_ref = decode(S, dec)
