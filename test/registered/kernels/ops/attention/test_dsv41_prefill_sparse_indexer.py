...
sparsely must pick the positions the dense masked top-k picks."""
...
def _publish(indexer, c):
    rows = c["lens"].numel()
    request_ids = torch.zeros(rows, dtype=torch.int32, device="cuda")
    return indexer.publish_prefill(
        c["dense"], c["lens"], c["page_table"], PAGE, request_ids, [rows], torch.int8
    )
...
@pytest.mark.parametrize("rows,ctx", [(256, 40000), (64, 3000)])
@torch.inference_mode()
def test_select_prefill_matches_dense_masked_topk(rows, ctx):
    """Sparse bf16 scores at the published blocks equal the dense fp32 scores
    up to bf16 rounding, and the selection agrees with the masked dense top-k
    except at rounding-boundary ties."""
    indexer = _indexer()
    c = _case(rows, ctx, seed=rows * 3 + ctx)
    table = _publish(indexer, c)
    positions = torch.empty(rows, TOPK, dtype=torch.int32, device="cuda")
    indexer.select_prefill(
        table,
        c["q_fp4"].view(rows, 1, HEADS, 64),
        c["q_sf"].view(rows, 1, HEADS),
        c["k_cache"],
        c["weights"],
        positions,
    )
    ...
    masked = c["dense"].masked_fill(~keep[:, : c["width"]], -torch.inf)
    expected = torch.empty_like(positions)
    topk_transform_ragged_v2(
        masked, c["lens"], out_offsets=c["ks"], out_indices=expected
    )
    expected = mask_topk_scores(masked, expected, c["ks"])
...
@torch.inference_mode()
def test_prefill_rows_rebuilds_the_tail():
    ...
    sub = indexer.prefill_rows(table, idx, [tail])
    ...
    full = torch.empty(rows, TOPK, dtype=torch.int32, device="cuda")
    indexer.select_prefill(
        table,
        c["q_fp4"].view(rows, 1, HEADS, 64),
        c["q_sf"].view(rows, 1, HEADS),
    ...
    part = torch.empty(tail, TOPK, dtype=torch.int32, device="cuda")
    indexer.select_prefill(
        sub,
        c["q_fp4"][idx].view(tail, 1, HEADS, 64),
        c["q_sf"][idx].view(tail, 1, HEADS),
        c["k_cache"],
        c["weights"][idx],
        part,
    )
